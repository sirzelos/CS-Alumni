...
if(isset($_POST['content-r'])){
  $query = "INSERT INTO report(news_id,user_id,comment) VALUES (:news_id,:user_id,:comment)";
  $run = $db->prepare($query);
  try {
      $run->execute(array('news_id'=> $_GET['news_id'] , 'user_id' => $id ,'comment' => $_POST['content-r']));
      $message = 'ส่งรายงานสำเร็จ';
  } catch (PDOException $e) {
      $message = 'ส่งรายงานไม่สำเร็จ';
  }
...
<div class="container">
  ...
  <div class="content-container">
    <?php
    $new_id = $_GET['news_id'];
    $name_query = "SELECT title FROM news WHERE id = $new_id";
    $name_run = $db->prepare($name_query);
    $name_run->execute();
    $title_name = $name_run->fetchColumn();
    ?>
    <h1 style="margin-left: 15px;margin-top:50px;">รายงานข่าวไม่เหมาะสม เรื่อง <?= $title_name ?></h1>
    <form class = "my-form" style="margin-top: 25px;" action="reportform.php?news_id=<?= $_GET['news_id'] ?>" method="post">
      <h3>เหตุผล</h3> <textarea class = "text-content" name="content-r" method="post" required></textarea>
      <hr>
